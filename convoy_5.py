...
def s3db_to_json(db_filename):
    ...
    convoy_dict = database.to_dict(orient='records')
    ...
    if len(convoy_dict) == 1:
        ...
        print(f'{len(convoy_dict)} vehicles were saved into {db_filename}')
    s3db_to_xml(conn, db_filename, database)


def s3db_to_xml(conn, db_filename, database):
